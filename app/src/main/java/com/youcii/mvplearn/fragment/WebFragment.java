...
public class WebFragment extends BaseFragment implements IFragWebView, View.OnClickListener {
    ...
    public void onStart() {
        ...
        webView.registerHandler("decNativeNum", (data, function) -> {
            if (data.contains("啊啊")) {
                nativeNum.setText(String.valueOf((Integer.parseInt(nativeNum.getText().toString()) - 1)));
            } else {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.numAdd:
                ...
                break;
            case R.id.numDec:
                webView.callHandler("decHtmlNum", "哦哦", ToastUtils::showShortToast);
                break;
            default:
                break;
        }
